 ...
 */
package com.rzico.weex.zhibo.animation;
...
public abstract class AbstractPathAnimator {
    ...
    public Path createPath1(AtomicInteger counter, View view, int factor) {
        Random r = mRandom;
        ...
        //p.cubicTo(x1, y1, x2, y2, x3, y3) (x1,y1) 为控制点，(x2,y2)为控制点，(x3,y3) 为结束点。
        p.cubicTo(mConfig.initX, y - factor, x, y2 + factor, x, y2);
        int mx=mConfig.initX;
        int my=y-factor;
        int my2=y2+factor;
        p.moveTo(x, y2);
    ...
    public Path createPath(AtomicInteger counter, View view, int factor) {
        ...
        if(mConfig.mytpye==1){//小范围的飘雪
            p.moveTo(x/2, y);
            p.cubicTo(x/2,y,x-r.nextInt(x), 3*y/4, 4*x/5,y/2);
            p.moveTo(x/2, y/2);
            p.cubicTo(x/2, y/2, x-r.nextInt(x), y/4, x/2,0);
        }else {
            p.moveTo(mConfig.initX, y);
            p.cubicTo(mConfig.initX, y, x-r.nextInt(x), 3*y/4, x*3/4, y/2);
            p.moveTo(x-r.nextInt(x), y/2);
            p.cubicTo(x-r.nextInt(x),  y/2, x-r.nextInt(x),y/4, x-r.nextInt(x),  y3);
//            p.moveTo(mConfig.initX, y);
//            p.cubicTo(mConfig.initX, y - factor, x, y2 + factor, x, y2);
//            p.moveTo(x, y2);
//            p.cubicTo(x, y2 - factor, x2, y3 + factor, x2, y3);
...
//                    res.getDimensionPixelOffset(R.dimen.heart_size_height));//动画图片高度
            ...
            config.mytpye = typedArray.getInteger(R.styleable.HeartLayout_mtype,2);
            return config;
        }
