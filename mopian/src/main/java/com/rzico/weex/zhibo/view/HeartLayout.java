...
import com.rzico.weex.zhibo.animation.AbstractPathAnimator;
import com.rzico.weex.zhibo.animation.PathAnimator;
...
public class HeartLayout extends RelativeLayout implements View.OnClickListener {
    ...
    ArrayList<HeartView> heartViewlist=new ArrayList<>();
    ...
    private void init(AttributeSet attrs, int defStyleAttr) {
        ...
        if (pointx <= initX && pointx >= 10) {
            pointx -= 10;
        } else if (pointx >= -initX && pointx <= 10) {
            pointx += 10;
        } else pointx = initX;
//        Resources res = a.getResources();
        int mytpp = a.getInteger(R.styleable.HeartLayout_mtype,2);

        mAnimator = new PathAnimator(AbstractPathAnimator.Config.fromTypeArray(a, initX, textHight, pointx, dWidth, dHeight),mytpp);
        a.recycle();
    }
    ...
    private static int[] drawableIds = new int[]{R.drawable.heart01, R.drawable.heart02,R.drawable.heart03, R.drawable.heart04, R.drawable.heart05, R.drawable.heart06, R.drawable.heart07, R.drawable.heart08,R.drawable.heart09,};
...
//        peri++;
     ...
     * @param height 图片高
     */
    public void addFavor(int i,int j,int width,int height) {
        dWidth=width;
        dHeight=height;
        if(i>=j) {
            clearAnimation();
        ...
        heartView.setDrawable(drawableIds[random.nextInt(8)]);
        init(attrs, defStyleAttr);
        mAnimator.start(heartView, heartLayout);
