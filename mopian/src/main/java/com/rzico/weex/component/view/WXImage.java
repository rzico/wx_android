...
public class WXImage extends WXComponent<GifImageView> {
  ...
  @Override
  protected GifImageView initComponentHostView(@NonNull Context context) {
  ...
  public void setSrc(String src) {
    ...
    if(src.endsWith(".gif")){
      ...
      try{
        GifImageView gifImageView = getHostView();
        if(src.startsWith("file://resource")){
          src =  src.replace("file://", PathUtils.getResPath(WXApplication.getActivity()));//读取资源路径
        }
        GifDrawable gifFromAssets = new GifDrawable(  src);
  ...
  public void recycled() {
    ...
    if (getInstance().getImgLoaderAdapter() != null) {
      getInstance().getImgLoaderAdapter().setImage(null, (ImageView) getRealView(),
              null, null);
    } else {
  ...
  public void autoReleaseImage(){
    if(mAutoRecycle){
      if(getHostView() != null){
        if (getInstance().getImgLoaderAdapter() != null) {
          getInstance().getImgLoaderAdapter().setImage(null, (ImageView) getRealView(), null, null);
        }
      }
    }
  }
  ...
  public void save(final JSCallback saveStatuCallback) {
    ...
    WXViewToImageUtil.generateImage((ImageView) getRealView(), 0, 0xfff8f8f8, new WXViewToImageUtil.OnImageSavedCallback() {
      @Override
